...
const DashVolunteer: NextPage<{ driveData: { drive: BookDrive | null, reactivationReq: ReactivationRequest | null }[], account: VolunteerAccount | null, error: Error | null }> = ({ driveData, account, error }) => {
  ...
  const submitReq = async (isEdited: boolean, id?: string) => {
    try {
      const response = isEdited ? await editReactivationRequest(id!, reactivationReqMessage) : await sendReactivationReq(currDriveCode, account!.alp_id, reactivationReqMessage)
      if (!response.success) throw new Error(response.error)
      isEdited ? toggleModal(viewReqModalRef, false, "") : toggleModal(newReqModalRef, false, "")
      const datum = isEdited ? driveData.find(driveDatum => driveDatum.drive?.reactivationRequestId === response.reactivationReq?.id) : driveData.find(driveDatum => driveDatum?.drive?.driveCode === response.reactivationReq.driveCode)
      ...
      setCurrReqId("")
      isEdited ? alert("Successfully edited reactivation request") : alert('Successfully sent request')
    } catch (e: Error | any) {
      console.error(e)
  ...
  const deleteReq = async (driveCode: string, reqId: string) => {
    try {
      ...
      const datum = driveData.find(driveDatum => driveDatum.drive?.reactivationRequestId === response.reactivationReq?.id)
  ...
  const toggleModal = (ref: React.RefObject<HTMLDialogElement>, val: boolean, driveCode: string) => {
    setCurrDriveCode(driveCode)
    setEditReq(false)
    if (val) ref?.current?.showModal();
    else ref?.current?.close()
  }
  const openViewReqModal = (reqId: string, driveCode: string, message: string) => {
    setCurrReqId(reqId)
        ...
        <Box display="flex" flexDirection="column" sx={{
          ...
          {driveData && <Stack
            ...
            {driveData.map((driveDatum) => (<>
              <DriveCard drive={driveDatum.drive} reqExists={driveDatum.reactivationReq ? true : false} reactivationReq={driveDatum.reactivationReq} openNewReqModal={() => toggleModal(newReqModalRef, true, driveDatum.drive?.driveCode!)} openViewReqModal={() => openViewReqModal(driveDatum.reactivationReq!.id, driveDatum.drive?.driveCode!, driveDatum.reactivationReq!.message)}></DriveCard>
            </>
            ))}
          </Stack>}
          ...
          <dialog ref={viewReqModalRef} style={{ height: "55%", width: "50%", borderRadius: "3%", padding: 0, position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)" }}>
            <Grid2 display={"flex"} flexDirection={"column"} justifyContent={"space-around"} alignItems={"center"} sx={{ backgroundColor: "#F5F5F5", width: "100%", height: "100%", padding: 2 }}>
              <p style={{ marginBottom: "2px", color: "#FE9834", fontWeight: 600, fontSize: 20 }}>Reactivation Request Message for {currDriveCode}</p>
              {editReq && currReqId && <TextField multiline autoFocus required placeholder="message" value={reactivationReqMessage} rows={8} sx={{ width: "85%", marginBottom: "5px", backgroundColor: "#FFFFFF", color: "#FE9384", borderColor: "#FE9834" }} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setReactivationReqMessage(e.target.value)} />}
              {!editReq && currReqId && <Grid2 sx={{ width: "85%", height: "85%", backgroundColor: "#FFFFFF", margin: 2, padding: 2, borderRadius: "3%" }}><p>{reactivationReqMessage}</p></Grid2>}
              <Grid2 display="flex" flexDirection="row" justifyContent="space-around" alignItems="center" sx={{ width: "65%" }}>
                <Button onClick={() => toggleModal(viewReqModalRef, false, "")} sx={{ backgroundColor: "#F3D39A", "&:hover": { backgroundColor: "#D3A874", }, fontWeight: 550, color: '#5F5F5F' }}>Close</Button>
