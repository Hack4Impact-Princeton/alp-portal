...
import {MessageGroup as MsgGroup} from '@chatscope/use-chat/dist/MessageGroup'
...
export const Chat = ({ user, email }: { user: User, email: string }) => {
    ...
    const [currMessages, setCurrMessages] = useState(currentMessages)
    ...
    const socketInitializer = async () => {
        ...
        socket.on('message', (message: ChatMessage<MessageContentType>, conversationId: string) => {
            ...
            let copy = currMessages
            //console.log(copy.length)
            const newMessageGroup = new MsgGroup({
                id: nanoid(),
                senderId: message.senderId,
                direction: message.direction,
              });
            //console.log('did we get here')
            newMessageGroup.addMessage(message)
            //console.log("what about here")
            copy.push(newMessageGroup)
            //console.log("what about this")
            setCurrMessages(copy)  
            console.log("updated current messages")            
        })
        return null
    ...
    return (<MainContainer responsive>
        ...
        <ChatContainer>
            ...
            <MessageList typingIndicator={getTypingIndicator()}>
                {activeConversation && currMessages.map((g) => {
                //console.log(g)
                return <MessageGroup key={g.id} direction={g.senderId == email ? 'outgoing' : 'incoming'}>
                    <MessageGroup.Messages>
                        {g.messages.map((m: ChatMessage<MessageContentType>) => {
                            //console.log(m.senderId == email ? 'outgoing': 'incoming')

                            return < Message key={m.id} model={{
                                type: "html",
                                payload: m.content,
                                direction: m.senderId == email ? 'outgoing' : 'incoming',
                                position: "normal"
                            }} />
                        })}
                    </MessageGroup.Messages>
