...
const ProfileDisplayCase: React.FC<{ profiles: ProfileInfoProps[]; useBadges?: boolean }> = ({ profiles, useBadges = true }) => {
  return (
    <Grid container style={{ border: '1.5px solid black', padding: '10px', marginBottom: '10px', display: 'flex', width: '95%', backgroundColor: "#F5F5F5" }}>
      ...
      <Grid container xs={12} style={{ display: 'flex', justifyContent: 'flex-start', flexWrap: 'wrap', paddingLeft: '10px' }}>
        {profiles.map((profile, index) => (
          <ProfileCard key={index} useBadges={useBadges} {...profile} style={{ marginRight: '20px', marginBottom: '20px' }} />
        ))}
      </Grid>
    </Grid>
  );
